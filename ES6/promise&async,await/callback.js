...
/** 멸망의 피라미드
 * 콜백 기반 비동기 처리는 언뜻 봤을 때 꽤 쓸만해 보이고, 실제로도 그렇다.
 * 한 개 혹은 두 개의 중첩 호출이 있는 경우는 보기에도 나쁘지 않다.
 * 
 ...
 * 
 * 호출이 계속 중첩되면서 코드가 깊어진다.
 * 중간중간의 ...로 표시한 곳에 반복문과 조건문이 있는 코드가 들어가면 관리는 특히나 더 힘들어질 것이다.
 * 
 ...
 * 따라서 이런 코딩 방식은 좋지 않다.
 * 
 * 각 동작을 독립적으로 함수로 만들어 위와 같은 문제를 완화할 수 있다.
 */
...
/* 새롭게 작성한 코드는 각 동작을 분리해 최상위 레벨의 함수로 만들었기 때문에 깊은 중첩이 없다.
 * 그리고 콜백 기반 스타일 코드와 동일하게 동작한다.
 * 그런데 이렇게 작성하면 동작상의 문제는 없지만, 코드가 찢어진 종잇조각 같아 보인다는 문제가 생긴다. 즉, 읽는 것이 어려워진다.
 * 눈을 이리저리 움직이며 코드를 읽어야 한다. 코드에 익숙지 않아 눈을 어디로 옮겨야 할지 모르면 더욱더 불편할 것이다.
 * 
 * 게다가 step*이라고 명명한 함수들은 '멸망의 피라미드'를 피하려는 용도만으로 만들었기 때문에 재사용이 불가능하다.
 * 그 누구도 연쇄 동작이 이뤄지는 코드 밖에선 함수들을 재활용하지 않을 것이다. 네임스페이스가 약간 복잡해졌다.(namespace cluttering)
 * 
 * 운 좋게도, 멸망의 피라미드를 피할 방법은 몇 가지 있다. 
 * 가장 좋은 방법 중 하나는 '프라미스(promise)'를 사용하는 것이다.
 */
