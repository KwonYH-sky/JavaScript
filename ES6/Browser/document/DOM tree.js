...
/** DOM 구조 직접 보기
 * 개발자 도구에서 DOM을 볼 수 있으며, 요소를 클릭하면 자세한 내용을 볼 수 있다.
 * 개발자 도구를 이용해 DOM 구조를 볼 땐, 생략된 부분이 있다는 점에 유의하자.
 * 텍스트 노드는 그냥 텍스트로만 표시되고, 띄어쓰기만 있는 빈 텍스트 노드는 나타나지 않는다.
 * 개발 중엔 대부분 요소 노드만 다루기 때문에 이 점이 문제가 되지는 않는다.
 * 
 * Element 패널 우측엔 여러 하위 패널이 있다.
    * Style - 내장 규칙(회색 배경)을 포함하여 현재 선택한 요소에 적용된 CSS 규칙을 일률적으로 보여준다.
        하단부 박스에 있는 크기(dimension), 마진(margin), 패딩(padding)을 비롯한 스타일 대부분을 Style 패널에서 바로 수정해 볼 수 있다.
    * Computed - 현재 선택한 요소에 적용된 CSS 규칙을 프로퍼티 기준으로 보여준다. CSS 상속 등을 통해 적용된 규칙도 볼 수 있다.
    * Event Listeners - DOM 요소에 붙은 이벤트 리스너를 볼 수 있다.
    * 기타 등등
 */
 ...
 * HTML/XML 문서는 브라우저 안에서 DOM 트리로 표현된다.
    ...
    * 문자는 텍스트 노드가 된다.
    * 이 외에 HTML 내의 모든 것은 DOM을 구성한다. 주석까지 포함
